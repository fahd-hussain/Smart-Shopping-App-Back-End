...
shortestPathRouter
        ...
        MAP._shelvesFromDatabase().then(() => {
            ...
            Lists.findById(req.params.listId)
                ...
                .then(
                    (list) => {
                        const itemNames = [];
                        list.listItems.map((item) => itemNames.push(item.itemName));
                            ...
                            .then((res) => {
                                ...
                                res.map((ele) => {
                                    if (!ele) {
                                        ...
                                        temp.push({ name: ele.name, shelf: ele.shelf.name });
                                    }
                                });
    ...
    .post(authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.end("POST operation not supported on /shortestPaths/" + req.params.shortestPathId);
    })
    .put(authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.end("PUT operation not supported on /shortestPaths/" + req.params.shortestPathId);
    })
    .delete(authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.end("DELETE operation not supported on /shortestPaths/" + req.params.shortestPathId);
    });
